...
while true; do
    read -p "Delete Kinesis stream ($STREAM)? (y/n)" response
    case $response in
        [Yy]* ) aws kinesis delete-stream --stream-name $STREAM; break;;
        [Nn]* ) break;;
        * ) echo "Response must start with y or n.";;
    esac
done
...
while true; do
    read -p "Delete Opensearch domain ($OPENSEARCH)? (y/n)" response
    case $response in
        [Yy]* ) aws opensearch delete-domain --domain-name $OPENSEARCH; break;;
        [Nn]* ) break;;
        * ) echo "Response must start with y or n.";;
    esac
done
